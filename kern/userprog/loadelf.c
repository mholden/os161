...
load_elf(struct vnode *v, vaddr_t *entrypoint)
{
	...
	/*
	 * Forget this, for now. Our address space doesn't really have any
	 * 'regions' to set up at this point. Later, we'll put in a region
	 * for the heap to make user-level malloc work, but again, for now
	 * let's keep it simple. No need for as_define_region().
	 */
	...
	/*
	 * We don't want to load any pages in here, either. On-demand paging,
	 * do it when we fault. No need for as_prepare_load() or load_segment().
	 */
	...
	*/
	...
	/*
	 * Yeah, all we really need from load_elf() at this point is the
	 * entry point for our program. Tell us where to start. We'll 
	 * immediately start faulting, but that's okay.
	 */
