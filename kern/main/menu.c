...
/*
 * Command for changing db flags dynamically.
 */
static
int
cmd_dbflags(int nargs, char **args)
{
	...
	switch(atoi(args[1])){
		case 1:		change = DB_LOCORE;
				break;
		case 2:		change = DB_SYSCALL;
				break;
		case 3:		change = DB_INTERRUPT;
				break;
		case 4:		change = DB_DEVICE;
				break;
		case 5:		change = DB_THREADS;
				break;
		case 6:		change = DB_VM;
				break;
		case 7:		change = DB_EXEC;
				break;
		case 8:		change = DB_VFS;
				break;
		case 9:		change = DB_SFS;
				break;
		case 10:	change = DB_NET;
				break;
		case 11:	change = DB_NETFS;
				break;
		case 12:	change = DB_KMALLOC;
				break;
		default:	kprintf("Usage: df nr on/off\n");
				return EINVAL;
	}
	
	if(!strcmp(args[2], "on")) dbflags = dbflags | change;
	else dbflags = dbflags & ~change;
...
{
	lock_acquire(pcb_fork_lock); // for synchronization with common_prog().. messy, but works
	...
	result = runprogram(progname, args, argc);
	if (result) {
...
common_prog(int nargs, char **args)
{
	...
	pid_t pid_for_prog = acquire_pid();

	if(pid_for_prog == -1){
		kprintf("aquire_pid() failed in thread 0x%x. Aborting common_prog().\n",
			(int) curthread);
		return -1;
	}
	...
	if (result) {
		lock_release(pcb_fork_lock);
		free_pid(pid_for_prog);
		kprintf("thread_fork failed: %s\n", strerror(result));
		return result;
	}

	struct pcb_node *new_node = insert_pcb_node(pid_for_prog, newthread);
	
	if(new_node == NULL){
		lock_release(pcb_fork_lock);
		free_pid(pid_for_prog);
		kprintf("insert_pcb_node() failed in thread 0x%x. Aborting cmd_progthread().\n",
			(int) curthread);
		return -1;
	}
	lock_release(pcb_fork_lock);
